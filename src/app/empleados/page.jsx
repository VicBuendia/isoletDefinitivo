  ...
  return (
    <div className='p-5 flex flex-col gap-5 mt-1  bg-[#f7f6f2]'>
      ...
      <div className="w-full flex mb-6 lg:w-1/2 lg:mx-80">
          <input type="text" name="busqueda" className="border rounded-l-3xl py-2 px-5 w-full placeholder:italic shadow-md" placeholder= 'Buscar por nombre o teléfono' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
          <button className='p-3 bg-zinc-800 rounded-r-3xl text-white hover:bg-gradient-to-b from-zinc-900 hover:to-zinc-700 shadow-md border-r border-t border-b' onClick={buscarEmpleados}>Buscar</button>
      </div>
      {isLoaded && (
