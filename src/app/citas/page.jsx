  ...
  return (
    <div className='p-5 flex flex-col gap-5  bg-zinc-100'>
      ...
      <div className="w-full flex gap-3 mb-6 lg:w-auto lg:mx-12">
          <input type="text" name="busqueda" className="border rounded-2xl py-2 px-5 w-full  placeholder:italic shadow-md" placeholder='Buscar por fecha, nombre o teléfono' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
          <button className='p-3 bg-zinc-800 rounded-xl text-white hover:bg-gradient-to-b from-zinc-900 hover:to-zinc-700 shadow-lg' onClick={buscarCitas}>Buscar</button>
      </div>
      {isLoaded && (
